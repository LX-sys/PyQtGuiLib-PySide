...
font-size:10px;
        ...
        self.ani = Animation()
        self.ani.addAni(self.btn.pos(),QPoint(300,300),courseFunc=self.btn.move,duration=5000,loopCount=2)
    ...
    def test(self):
        print(self.ani.state())
        if self.ani.state() == Animation.Running:
            self.ani.paused()
        else:
            self.ani.resume()
